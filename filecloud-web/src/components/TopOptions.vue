...
const shareClick = () => {
    //获取分享链接
    const selectedRow = selectedTableRow.value;
    const dynamicUrlStr = selectedRow.dynamicDownloadUrl;
    //将链接复制进剪切板
    if (window.isSecureContext && navigator.clipboard) {
        copyToClipboard_https(dynamicUrlStr);
    } else {
        copyToClipboard_http(dynamicUrlStr);
    }
} 
//在安全上下文的剪切板复制操作
const copyToClipboard_https = (dynamicUrlStr) => {
    // 在安全上下文中使用 clipboard API
    navigator.clipboard.writeText(dynamicUrlStr).then(() => {
...
//在非安全上下文的剪切板复制操作
const copyToClipboard_http = (dynamicUrlStr) => {
    // 非安全上下文中使用 document.execCommand 作为回退方案
    const textarea = document.createElement('textarea');
    textarea.value = dynamicUrlStr;
